    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#table-pembayaran').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'pendaftaran.customer.nama_lengkap',
                    },
                    {
                        data: 'status',
                    },
                    {
                        data: 'pendaftaran.customer.notelp',
                    },
                    {
                        data: 'id',
                        "render": function(data, type, row) {
                            let uriShow =
                                "{{ route('pembayaran.show', ['pembayaran' => ':id', 'type' => ':tipe']) }}"
                                .replace(
                                    ':id', data).replace(':tipe', row.pendaftaran.type);

                            return `<div class="d-flex">
                        			<a href="${uriShow}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>
                        		</div>`
                        }
                    }
                ],
        ...
        function deleteData(id) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    let uriDelete = "{{ route('pembayaran.destroy', ['pembayaran' => ':id']) }}".replace(':id', id);
                    $.ajaxSetup({
                        headers: {
